...
class Frontier_Explorer(Node):
    def __init__(self):
        ...
        self.detector_sub = self.create_subscription(
            Bool, "/detector_bool", self.detector_cb, 10
        )
    ...
    def explore(self):
        ...
        ) / (window_size **2)  # TODO define the kernel
        occupied = convolve2d(
            occupied_mask.astype(float), kernel, mode="same"
        )  # TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unoccupied = convolve2d(
            unoccupied_mask.astype(float), kernel, mode="same"
        )  # TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unknown = convolve2d(
            unknown_mask.astype(float), kernel, mode="same"
        )  # TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        ...
        frontier_state = np.argmin(
            np.linalg.norm(frontier_states - current_state, axis=1)
        )
        frontier_state = frontier_states[frontier_state, :]

        msg = TurtleBotState()  # TODO
        msg.x, msg.y = frontier_state[0], frontier_state[1]  # TODO
    ...
    def stop_sign_wait(self):
        ...
        if (self.current_time - self.prev_time) <= 5:
            ...
            self.set_parameters([rclpy.Parameter("active", value=True)])
            self.stop_flag = (
                1  # stop sign has been detected and being dealt with (end process)
            )
