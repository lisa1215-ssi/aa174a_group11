...
class Frontier_Explorer(Node):
    ...
    def explore(self):
		...
		unknown_mask = np.where(self.occupancy.probs < 0, 1, 0)# TODO use np.where to find unknown mask which is indicated by the value -1
		unoccupied_mask = np.where(self.occupancy.probs >= 0 & self.occupancy.probs < 0.5, 1, 0)# TODO use np.where to find unknown mask which is indicated by values in range [0, 0.5)

        kernel = np.ones((window_size, window_size), dtype=np.float32) # TODO define the kernel
        occupied = convolve2d(occupied_mask.astype(float), kernel, mode="same")# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unoccupied = convolve2d(unoccupied_mask.astype(float), kernel, mode="same")# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unknown = convolve2d(unknown_mask.astype(float), kernel, mode="same")# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'

        total_count = convolve2d(np.ones_like(self.occupancy.probs, dtype=float), kernel, mode="same")

        with np.errstate(divide='ignore', invalid='ignore'):
            unknown_pct = np.where(total_count > 0, unknown / total_count, 0.0)
            unoccupied_pct = np.where(total_count > 0, unoccupied / total_count , 0.0)

        cond1 = unknown_pct >= 0.2
        cond2 = occupied == 0
        cond3 = unoccupied_pct >= 0.3

        frontier_mask = cond1 & cond2 & cond3 & unoccupied_mask # TODO use np.where to make frontier mask based on the 3 conditions of Exploration Heuristics
        ...
        msg = "Finished exploring! There are no states left to explore :)"# TODO
        # msg.x, msg.y = #TODO
